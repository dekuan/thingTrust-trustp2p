...
const _scrypt				= require( 'scrypt' );
const _assert				= require( 'assert' );
const _hdkey				= require( 'hdkey' );
const { randomBytes, createHash }	= _crypto;
...
{
	return _crypto.randomBytes( _constants.B / 8 );
};
...
{
	return Buffer.isBuffer( key ) && key.length === _constants.B / 8;
};


/**
 *	Calculate the distance between two keys
 *	@param {string} id1 - Identity key to compare
 *	@param {string} id2 - Identity key to compare
 *	@returns {buffer}
 */
exports.getDistance = function ( id1, id2 )
{
	...
	_assert( exports.keyBufferIsValid( id1 ), 'Invalid key supplied' );
	_assert( exports.keyBufferIsValid( id2 ), 'Invalid key supplied' );

	return Buffer( _constants.B / 8 ).map( ( b, index ) => id1[ index ] ^ id2[ index ] );
};
...
{
	_assert( exports.keyBufferIsValid( b1 ), 'Invalid key supplied' );
	_assert( exports.keyBufferIsValid( b2 ), 'Invalid key supplied' );
...
{
	_assert( exp >= 0 && exp < _constants.B, 'Index out of range' );
	...
	buffer[ _constants.K - byteValue - 1 ] = 1 << ( exp % 8 );
...
exports.getRandomBufferInBucketRange = function( referenceKey, index )
{
	...
	for ( let i = _constants.K - 1; i > ( _constants.K - byte - 1 ); i-- )
	{
	...
	for ( let j = index - 1; j >= byte * 8; j-- )
	{
		...
		base[ _constants.K - byte - 1 ] |= one ? ( 1 << shiftAmount ) : 0;
	}
...
{
	_assert( typeof storage === 'object', 'No storage adapter supplied' );
	_assert( typeof storage.get === 'function', 'Store has no get method' );
	_assert( typeof storage.put === 'function', 'Store has no put method' );
	_assert( typeof storage.del === 'function', 'Store has no del method' );
	_assert( typeof storage.createReadStream === 'function', 'Store has no createReadStream method' );
};
...
{
	_assert( typeof logger === 'object', 'No logger object supplied' );
	_assert( typeof logger.debug === 'function', 'Logger has no debug method' );
	_assert( typeof logger.info === 'function', 'Logger has no info method' );
	_assert( typeof logger.warn === 'function', 'Logger has no warn method' );
	_assert( typeof logger.error === 'function', 'Logger has no error method' );
};
...
{
	_assert( typeof transport === 'object', 'No transport adapter supplied' );
	_assert( typeof transport.read === 'function', 'Transport has no read method' );
	_assert( typeof transport.write === 'function', 'Transport has no write method' );
};
...
{
	return _crypto.createHash( 'sha256' ).update( input ).digest();
};
...
module.exports.scrypt = function( input, callback )
{
	...
	{
		return _scrypt.hash( input, params, length, salt, callback );
	}
	else
	{
		return _scrypt.hashSync( input, params, length, salt );
	}
};
...
{
	return _crypto.createHash( 'rmd160' ).update( input ).digest();
};
...
{
	const { protocol, hostname, port, hash }	= _url.parse( addr );
	const contact = [
...
module.exports.isCompatibleVersion = function( version )
{
	...
	const sameMajor = _semver.major(local) === _semver.major(remote);
	const diffs = ['prerelease', 'prepatch', 'preminor', 'premajor'];

	if ( diffs.indexOf( _semver.diff( remote, local ) ) !== -1 )
	{
		return false;
	}
	else
...
{
	const [, info]		= contact;
	const isValidAddr	= _ip.isV4Format( info.hostname ) ||
				_ip.isV6Format( info.hostname ) ||
				_ip.isPublic( info.hostname );
	const isValidPort	= info.port > 0;
	const isAllowedAddr	= _ip.isLoopback( info.hostname ) ? !! loopback : true;
 ...
 */
module.exports.isValidNodeIndex = function( hdIndex )
{
	return ! Number.isNaN( hdIndex ) &&
		( parseInt( hdIndex ) === hdIndex ) &&
		hdIndex >= 0 &&
		hdIndex <= _constants.MAX_NODE_INDEX;
};
...
{
	const hdKeyPair	= _hdkey.fromMasterSeed( masterSeed || randomBytes( 64 ) );
 ...
 *	@returns {boolean}
 */
module.exports.isDerivedFromExtendedPublicKey = function( pub, xpub, i, p )
{
	let hdKeyPair	= _hdkey.fromExtendedKey( xpub );

	if ( p )
	{
		hdKeyPair = hdKeyPair.derive( p );
	}
