...
class KademliaRules
{
	...
	store( request, response, next )
	{
		...
		try
		{
			assert( typeof item === 'object', 'Invalid storage item supplied' );
			assert( typeof item.timestamp === 'number', 'Invalid timestamp supplied' );
			assert( utils.keyStringIsValid( item.publisher ), 'Invalid publisher identity supplied' );
			assert( utils.keyStringIsValid( key ), 'Invalid item key supplied' );
			assert( typeof item.value !== 'undefined', 'Invalid item value supplied' );
		}
		catch ( err )
		{
			return next( err );
		...
		this.node.storage.put( key, item, { valueEncoding : 'json' }, ( err ) =>
		{
			...
			//	NB: Echo back what was stored
			response.send( [ key, item ] );
		});
	}
