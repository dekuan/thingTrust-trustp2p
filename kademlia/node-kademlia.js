...
class KademliaNode extends AbstractNode
{
	...
	expire( callback = () => null )
	{
		...
		{
			if ( ( value.timestamp + _constants.T_EXPIRE ) <= now )
			{
	...
	refresh( startIndex = 0, callback = () => null )
	{
		...
		(
			...
			( index, next ) =>
			{
				...
				const needsRefresh	= lastBucketLookup + _constants.T_REFRESH <= now;
	...
	_iterativeFind( method, key, callback )
	{
		...
		(
			key,
			[
				...this.router.getClosestContactsToKey( key, _constants.ALPHA )
			]
		);
		let closest = shortlist.closest;

		this._lookups.set( _utils.getBucketIndex( this.identity, key ), Date.now() );

		function iterativeLookup( selection, callback, continueLookup = true )
		{
			if ( ! selection.length )
			{
				return callback( null, shortlist.active.slice( 0, _constants.K ) );
			}
			...
			(
				...
				() =>
				{
					...
					if ( shortlist.active.length >= _constants.K ||
					...
					if ( closest[ 0 ] === shortlist.closest[ 0 ] )
					{
						...
						(
							this,
							shortlist.uncontacted.slice( 0, _constants.K ),
							callback,
							false
						);
					}
					...
					(
						this,
						shortlist.uncontacted.slice(0, _constants.ALPHA),
						callback,
						true
					);
				}
	...
	_updateContactWorker( task, callback )
	{
		...
		(
			...
			( err ) =>
			{
				...
				(
					headId,
					{ timestamp : Date.now(), responded: ! err }
				);
